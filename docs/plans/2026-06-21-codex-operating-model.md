...
- [ ] Define authoritative artifact locations and promotion rules.
- [ ] Define templates with required headings.
- [ ] Define versioned evaluation scenarios with prompts, assertions, and
      prohibited behaviors.
- [ ] Run JSON parsing and Markdown heading checks.
- [ ] Commit as `docs: define Codex knowledge and evaluation contracts`.
...
- [ ] Write tests for valid documents, missing headings, invalid scenario JSON,
      and forbidden tracked raw-session files.
- [ ] Run `python3 -m unittest tests/test_validate_repository.py -v` and verify
      failure because the validator does not exist.
- [ ] Implement the minimum validator using only the Python standard library.
- [ ] Run `python3 -m unittest tests/test_validate_repository.py -v` and verify
      all tests pass.
- [ ] Run `python3 scripts/validate_repository.py .`.
- [ ] Commit as `build: validate project knowledge artifacts`.
...
- [ ] Run the unchanged scenario in a fresh `codex exec --ephemeral` session.
- [ ] Record raw JSONL under `.local/paios-sessions/evals/`.
- [ ] Score the baseline. If it passes, record that no skill is needed and stop.
- [ ] If RED, initialize and write the minimum skill addressing observed gaps.
- [ ] Validate the skill with `quick_validate.py`.
- [ ] Re-run the identical scenario explicitly using the skill and record GREEN.
- [ ] Commit as `feat: add evaluated PAIOS project workflow skill`.
...
- [ ] Run the unchanged session-close scenario in a fresh session.
- [ ] Score whether it creates a valid curated summary without modifying
      authoritative documents or tracking raw events.
- [ ] If RED, create the minimum session-close skill.
- [ ] Validate and re-run the identical scenario for GREEN.
- [ ] Commit as `feat: add evaluated PAIOS session close skill`.
...
- [ ] Write tests for safe session names, local output paths, event metrics, and
      final-message extraction.
- [ ] Run tests and verify RED because the utility is absent.
- [ ] Implement capture around `codex exec --json` using the standard library.
- [ ] Run unit tests and a read-only smoke session.
- [ ] Confirm raw JSONL is ignored and no secrets or transcripts are staged.
- [ ] Commit as `feat: capture Codex session evidence locally`.
...
- [ ] Document role-focused sessions, artifact promotion, RED–GREEN capability
      changes, and required verification.
- [ ] Add operating-model and command links to README.
- [ ] Run all unit tests, repository validation, `git diff --check`, and skill
      validation.
- [ ] Review the complete diff for privacy, portability, and unnecessary scope.
