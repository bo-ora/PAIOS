...
| TD-008 | Telegram | `medium` | `open` | **Phase 3.1 conversational-UX polish backlog** — post-acceptance walkthrough (2026-06-24) surfaced real usability gaps that do not break the grounding guarantee but reduce daily usefulness: (a) the recency list shows record ids + filenames, not human-readable content snippets; (b) ask-vs-capture is undiscoverable — a natural question in grounded mode is silently captured as a note, and bare `help`/`start` are not recognized (only `/help`); (c) no record-delete CLI command exists, so accidental/junk captures are permanent; (d) grounded ask is lexical, not recency-aware, so "my last voice note" cannot resolve "last"; (e) lexical retrieval misses morphological variants ("freedom" vs "feel free", "dancing" vs "dance"); (f) assist multi-turn context can bleed across unrelated topic switches; (g) summaries of Ukrainian sources render in English. | When a Phase 3.1 polish pass is scheduled, or any item starts blocking daily use. | Triage into a Phase 3.1 plan: snippet-ize recall + hide ids; accept bare `help`/question-phrasing hint; add a guarded record-delete; wire recency into ask or route "last/recent X" questions through `listRecords`; consider stemming/normalization for lexical recall; bound assist context on topic change; strengthen same-language summary. Evidence: `docs/sessions/2026-06-24-phase-3-conversational-recall.md`. |
